# train_olmotaur.py
"""
Olmotaur: SFT Training for OLMo Models on Psych-101

Replicates the Centaur paper methodology using OLMo base models.
Includes Weights & Biases (wandb) integration for experiment tracking.

Unlike Llama/Qwen/SmolLM, OLMo is NOT hosted on unsloth's model hub, so the
models are pulled directly from allenai. Unsloth's FastLanguageModel loads
them through its generic (transformers-backed) path as long as the installed
transformers version knows the architecture:
    * Olmo2ForCausalLM (OLMo-2-0425-1B)  -> transformers >= 4.47
    * Olmo3ForCausalLM (Olmo-3-1025-7B)  -> transformers >= 4.57

Both models are pretrained base models (no instruction tuning / RL).

Context-length caveat: OLMo-2-0425-1B was pretrained with a 4096-token
context, so its max_seq_length defaults to 4096 (longer Psych-101 sessions
get truncated). Olmo-3-1025-7B natively supports 65536 tokens and uses the
standard Centaur 32768. Override with --max_seq_length at your own risk.

Usage:
    # Basic training
    python train_olmotaur.py --size 1B

    # With wandb tracking
    python train_olmotaur.py --size 1B --wandb --wandb_project olmotaur --wandb_run olmo-1b-run1

    # With 4-bit quantization
    python train_olmotaur.py --size 7B --load_in_4bit
...
# Both entries are pretrained base models
# (no `-Instruct` / `-SFT` / `-DPO` variants).
# max_seq_length is capped at each model's native context:
# * OLMo-2-0425-1B was pretrained at 4096 tokens
# * Olmo-3-1025-7B natively supports 65536, so we use the
...
def get_batch_config(size: str, load_in_4bit: bool, num_gpus: int):
    ...
    and quantization, based on 1 x A100 GPU. The 1B model trains at a
    4096-token context, so it fits larger per-device batches than its
    parameter count alone would suggest.
    """
    if load_in_4bit:
        # 4-bit quantization - fits on fewer GPUs
        configs = {
            "1B": {"batch_size": 4, "grad_accum": 8, "device_map": "auto"},
            "7B": {"batch_size": 1, "grad_accum": 32, "device_map": "auto"},
        }
    else:
...
def train(
    ...
    wandb_project: str = "olmotaur",
    wandb_run_name: str = None,
    wandb_entity: str = None,
    wandb_tags: List[str] = None,
):
    """
    Train Olmotaur model on Psych-101 dataset.

    Args:
        size: Model size ("1B", "7B")
        output_dir: Output directory (default: ./outputs/olmotaur-{size})
        load_in_4bit: Use 4-bit quantization (QLoRA)
        num_gpus: Number of GPUs to use
        max_seq_length: Override the per-model default max sequence length
            (4096 for OLMo-2-1B, 32768 for Olmo-3-7B). Values above the
            native context are not RoPE-scaled through unsloth's generic
            loading path — override with care.
        ...
        lora_rank: Override the default LoRA rank (16 for every size in
            OLMO_CONFIGS). With rsLoRA, lora_alpha is kept equal to rank.
    ...
    if size not in OLMO_CONFIGS:
        raise ValueError(f"Invalid size: {size}. Choose from {list(OLMO_CONFIGS.keys())}")

    model_config = OLMO_CONFIGS[size]
    ...
    if output_dir is None:
        ...
        output_dir = f"./outputs/olmotaur-{size.lower()}{rank_suffix}{frac_suffix}{quant_suffix}"
    ...
    if use_wandb:
        try:
            ...
            if wandb_run_name is None:
                ...
                wandb_run_name = f"olmo-{size.lower()}-{quant_str}"
            ...
            wandb.init(
                ...
                name=wandb_run_name,
                entity=wandb_entity,
                tags=wandb_tags or [f"olmo-{size}", "olmotaur", "psych-101"],
                config={
                    "model_family": "olmo",
                    "model_size": size,
    ...
    # ============================================
    # Load Model
    # OLMo is not on unsloth's hub; FastLanguageModel loads it via the
    # generic transformers path (requires transformers with Olmo2/Olmo3
    ...
    # ============================================
    # Tokenizer Setup
    # OLMo-2/3 ship a dedicated <|pad|> token (id 100277) distinct from
    # eos, so we keep it instead of the pad_token_id = 0 trick used for
    # Llama/Qwen. Fall back to 0 only if pad is missing or aliases eos.
    # ============================================
    ...
    # (OLMo's dolma2 tokenizer doesn't add BOS, so the Llama-style
    # [1:] trick would drop a real token)
    # ============================================
    ...
    print(f"\nOlmotaur-{size} training complete!")
    print(f"Model saved to {output_dir}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train Olmotaur (OLMo) on Psych-101",
...
Examples:
  ...
  # With 4-bit quantization (recommended for 7B)
  python train_olmotaur.py --size 7B --load_in_4bit

  # With wandb tracking
  python train_olmotaur.py --size 1B --wandb --wandb_project my-project

  # Full example with all options
  python train_olmotaur.py --size 7B --load_in_4bit --num_gpus 2 \\
      --wandb --wandb_project olmotaur --wandb_run olmo-7b-experiment \\
    ...
    parser.add_argument(
        ...
        required=True,
        choices=["1B", "7B"],
        help="Model size: 1B (OLMo-2-0425-1B) or 7B (Olmo-3-1025-7B)"
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default=None,
        help="Output directory (default: ./outputs/olmotaur-{size})"
    )
    parser.add_argument(
        "--load_in_4bit",
        action="store_true",
        help="Use 4-bit quantization (QLoRA). Recommended for the 7B model."
    )
    parser.add_argument(
        "--num_gpus",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Override max sequence length (default: 4096 for 1B, 32768 for 7B). "
             "Values above the model's native context are not RoPE-scaled."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="olmotaur",
        help="Wandb project name (default: olmotaur)"
    )
    parser.add_argument(
        "--wandb_run",
        type=str,
